			...
			case "edit":
				if(isset($_SESSION['id']) && $this->args['srank']==1 && isset($vars['id'])) {
					if(isset($_POST['nick'])) {
						...
						$form->addInput("input", "text", "nick", "Nick: ".$user->getNick()."</br>Nowy nick:",$user->getNick())
							->addInput("input", "text", "email", "</br>Email: ".$user->getEmail()."</br>Nowy email:",$user->getEmail())
							->addInput("input", "text", "lvl", "</br>Lvl: ".$user->getLvl()."</br>Nowy lvl:",$user->getLvl())
							->addInput("select", null, "rank", "</br>Ranga: ".$user->getRankName()."</br>Nowa ranga:",array(
									"1" => "Administrator",
									"2" => "Użytkownik",
									"4" => "Zbanowany"
							))
		...
		switch ($vars['action']) {
				...
				if(isset($_POST['type'])) {
					if(isset($_POST['edit'])) {
						if($_POST['type']=="menu") {
							...
							if($_POST['name']!=$menu->getName()) {
								$menu->setName($_POST['name']);
							}
							if($_POST['rank']!=$menu->getRank()) {
								$menu->setRank($_POST['rank']);
							}
							$menus[$_POST['id']] = $menu;
			...
			case "panel":
				if(isset($_POST['edit'])) {
					...
					$form->addInput("input", "text", "title", "Tytuł Strony:",$title);
					$form->addInput("input", "hidden", "edit", null, true);
					echo $this->renderPage("form", $form);
				}
			break;
			case "list":
